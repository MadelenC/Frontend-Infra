...
export default function CreateJobApplicationForm({
  ...
  if (!formData.accesorio_id) {
    toast.warning("Seleccione un accesorio ⚙️");
    return;
  }
  ...
  const payload = {
    ...
    accesorio_id: formData.accesorio_id,
    descripsoli: desc,
  };
            ...
            <Select
                value={
                  vehiculos
                    .map(v => ({
                      value: v.id,
                      label: [v.tipog, v.placa].filter(Boolean).join(" - ")
                    }))
